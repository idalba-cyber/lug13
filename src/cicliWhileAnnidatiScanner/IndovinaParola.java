...
public class IndovinaParola {
	...
	public static void main(String[] args) {
		...
		while (!indovina.equals(parola.toLowerCase())) { //parola trasformata in minuscolo per ridurre il numero tentativi
			
			//se indovina non è parola
			indovina = ""; // resetto indovina
			
			// ciclo di creazione di indovinada confrontare
			while (indovina.length() < parola.length()) {
				//Java deve scegliere una lettera a caso avente codice ASCII compreso tra 97 e 122; 
				//abbiamo a disposizione il metodo random() che sorteggia un numero compreso tra 0 incluso e 1 escluso (da 0 a 0.9 periodico)
		...
		System.out.println(parola + " trovata dopo " + numeroTentativi + " tentativi");
				input.close(); // va chiusa l'istanza di Scanner
